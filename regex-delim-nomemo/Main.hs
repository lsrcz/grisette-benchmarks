...
instance (Mergeable1 m, Mergeable a) => Mergeable (Thread m a) where
  rootStrategy =
    SortedStrategy
      (\case Done -> False; Resume {} -> True)
      ( \case
          False -> SimpleStrategy $ \_ t _ -> t
          True -> product2Strategy Resume (\(Resume l r) -> (l, r)) rootStrategy rootStrategy1
      )
...
plusPatt patt greedy str idx =
  delimitCoro . pipe (patt str idx) $
    lift >=> \i ->
      mrgIf
        greedy
        (when (i /= idx) (callCoro (plusPatt patt (con True) str i)) >> yield (mrgReturn i))
        (yield (mrgReturn i) >> when (i /= idx) (callCoro (plusPatt patt (con False) str i)))
