...
data SysCall
  ...
  deriving (Mergeable, EvaluateSym) via (Default SysCall)

removeDisabledSyncs :: [SysCall] -> [SysCall]
removeDisabledSyncs [] = []
removeDisabledSyncs (Efsync _ v : xs) | v == con False = removeDisabledSyncs xs
removeDisabledSyncs (x : xs) = x : removeDisabledSyncs xs

newtype GenEfsync = GenEfsync Integer

instance GenSym GenEfsync SysCall

instance GenSymSimple GenEfsync SysCall where
  simpleFresh (GenEfsync n) = do
    fds <- chooseFresh [0 .. n - 1]
    b <- simpleFresh ()
    return $ Efsync fds b
...
instance Mergeable InodeOp where
  rootStrategy = NoStrategy

$(makeUnionWrapper "u" ''InodeOp)
