...
data DirEnt = DirEnt {dirEntIno :: UnionM Integer, dirEntExists :: SymBool}
  deriving (Show, Eq, Generic)
  deriving (Mergeable, ToSym ConcDirEnt, SimpleMergeable, EvaluateSym) via (Default DirEnt)
...
data File = File {fileSize :: UnionM Integer, fileOnDisk :: UnionM [SymBool]}
  deriving (Show, Eq, Generic)
  deriving (Mergeable, ToSym ConcFile, SimpleMergeable, EvaluateSym) via (Default File)
...
data Ext4Fs = Ext4Fs
  ...
  deriving (Show, Eq, Generic)
  deriving (ToSym ConcExt4Fs, EvaluateSym) via (Default Ext4Fs)

instance Mergeable Ext4Fs where
  rootStrategy = SimpleStrategy $ \cond (Ext4Fs bs1 na1 dir1 fds1 files1) (Ext4Fs _ _ dir2 fds2 files2) ->
    -- assume bs1 == bs2
    -- assume na1 == na2
...
-- $(makeUnionWrapper "u" ''Ext4FsCrackState)
    ...
    execStateT
      ( case i of
          IDirAdd nm fd -> do
            ...
            updateDirEnt nm $ DirEnt ino (con True)
            updateFdIno fd ino
          IDirRename name1 name2 -> unless (name1 == name2) $ do
            ...
            updateDirEnt name1 $ DirEnt oldIno (con False)
            updateDirEnt name2 $ DirEnt newIno (con True)
      ...
      doWrite fd content off = do
        ...
        let onDiskUpdated = do
              ...
              if fromIntegral off > length o
                then mrgReturn $ o ++ replicate (fromIntegral off - length o) (con False)
                else mrgReturn o
        let headAndTail = do
              o <- onDiskUpdated
    ...
    let DirEnt ino exists = dirs !! fromIntegral nm
     in mrgIf
          ...
          ( do
              ...
              s <- sz
              ods <- od
              mrgJust $ take (fromIntegral s) ods
          )
          mrgNothing
