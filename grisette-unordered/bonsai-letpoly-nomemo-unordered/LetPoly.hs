...
refTyU :: UUnionM LetPolyTree -> LetPolyTree
refTyU = BonsaiNode (mrgReturn $ simpleNode "ref")

arrowTyU :: UUnionM LetPolyTree -> UUnionM LetPolyTree -> LetPolyTree
arrowTyU = BonsaiNode

tyassert :: SymBool -> ExceptT BonsaiError UUnionM ()
...
typeCompatible :: LetPolyTree -> LetPolyTree -> ExceptT BonsaiError UUnionM ()
...
derefTy :: LetPolyTree -> ExceptT BonsaiError UUnionM (UUnionM LetPolyTree)
derefTy = tyMatch [letPolyLiteral "ref" *= placeHolder ==> return]

typer' :: LetPolyTree -> Env LetPolyWidth LetPolyTree -> ExceptT BonsaiError UUnionM (UUnionM LetPolyTree)
typer' tree env =
  tyMatch
...
typer :: LetPolyTree -> ExceptT BonsaiError UUnionM (UUnionM LetPolyTree)
...
data LetPolyValue
  ...
  deriving (GEvaluateSym Model, GSEq SymBool) via (Default LetPolyValue)
...
newtype RefEnv = RefEnv [(Integer, UUnionM (Maybe (UUnionM LetPolyValue)))]
...
updateRefEnv :: Integer -> UUnionM LetPolyValue -> RefEnv -> RefEnv
...
getRefEnv :: Integer -> RefEnv -> ExceptT BonsaiError UUnionM (UUnionM LetPolyValue)
...
type EvalType =
  ...
  RefEnv ->
  LetPolyTree ->
  ExceptT BonsaiError UUnionM (UUnionM LetPolyValue, RefEnv)
...
eval :: LetPolyTree -> ExceptT BonsaiError UUnionM (UUnionM LetPolyValue)
eval tree = do
...
execLetPoly :: LetPolyTree -> ExceptT BonsaiError UUnionM (UUnionM LetPolyValue)
execLetPoly tree = do
