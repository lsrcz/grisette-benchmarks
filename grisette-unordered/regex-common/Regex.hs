...
data Patt
  = PrimPatt Char
  | SeqPatt (UUnionM Patt) (UUnionM Patt)
  | AltPatt (UUnionM Patt) (UUnionM Patt)
...
matchFirst :: RegexSynth tag => proxy tag -> PattCoroType tag -> B.ByteString -> MaybeT UUnionM (Int, Int)
...
synthesisRegexCompiled :: RegexSynth tag => proxy tag -> GrisetteSMTConfig b -> UUnionM Patt -> PattCoroType tag -> B.ByteString -> [B.ByteString] -> IO (Maybe ConcPatt)
