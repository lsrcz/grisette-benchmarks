...
verifyEENITranslation (Left AssertionViolation) = con True
verifyEENITranslation _ = con False
...
verifyEENI config end indistinguishable steps progSpec =
  let p0 = genSymSimple progSpec "a"
      ...
      res = runExceptT $ do
        symAssume (indistinguishable m0 p0 m1 p1)
        m0k <- r0
        m1k <- r1
        symAssume (end m0k p0)
        symAssume (end m1k p1)
        symAssert (indistinguishable m0k p0 m1k p1)
   in do
        _ <- timeItAll "evaluate1" $ runExceptT r0 `deepseq` return ()
        _ <- timeItAll "evaluate2" $ runExceptT r1 `deepseq` return ()
        m <- timeItAll "Lowering/Solving" $ solveExcept config verifyEENITranslation res
        case m of
          Left _ -> do return Nothing
          Right mo -> return $ Just $ EENIWitness (evaluateSym True mo p0) (evaluateSym True mo p1)
...
main = timeItAll "Overall" $ do
  {-
    ...
    print $ step 1 (freshMachine 2) [uPush $ PCValue 1 (con False)]
    print $
      step
        2
        (freshMachine 2)
        [ uPush $ PCValue 1 (con False),
          uPush $ PCValue 1 (con True)
        ]
    print $
      step
        2
    ...
    let p3 =
          [ uPush $ PCValue 5 (con False),
            uCall (PCValue 0 (con False)) (PCValue 1 (con False)),
            uPush $ PCValue 1 (con False),
            uStoreCR,
            uHalt,
            uPush $ PCValue 2 (con True),
            uReturn
          ]
    ...
    let p5 =
          [ uPush $ PCValue 5 (con False),
            uCall (PCValue 0 (con False)) (PCValue 1 (con False)),
            uPush $ PCValue 0 (con False),
            uStoreCR,
            uHalt,
            uPush $ PCValue 0 (con False),
            uPush $ PCValue 9 (con True),
            uCall (PCValue 0 (con False)) (PCValue 0 (con False)),
            uPop,
            uPush $ PCValue 0 (con False),
            uReturn
          ]
    ...
    let p7 =
          [ uPush $ PCValue 5 (con True),
            uPush $ PCValue 7 (con False),
            uCall (PCValue 1 (con False)) (PCValue 0 (con False)),
            uHalt,
            uPop,
            uPush $ PCValue 4 (con False),
            uReturn,
            uCall (PCValue 0 (con False)) (PCValue 1 (con False)),
            uPush $ PCValue 1 (con False),
            uStoreCR
