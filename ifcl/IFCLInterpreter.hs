...
evalFailOn :: SymBool -> ExceptT Errors UnionM ()
evalFailOn = symAssertTransformableError EvalError
...
noopInst :: Machine -> ExceptT Errors UnionM Machine
noopInst m = mrgReturn $ next m

pushInst :: PCValue -> Machine -> ExceptT Errors UnionM Machine
pushInst p = mrgReturn . next . push (MPCValue p)

popInst :: Machine -> ExceptT Errors UnionM Machine
popInst = mrgFmap next . pop

popCRInst :: Machine -> ExceptT Errors UnionM Machine
popCRInst m = do
  _ <- peekPC 0 m
  mrgFmap next $ pop m

loadInst :: Machine -> ExceptT Errors UnionM Machine
loadInst m = do
...
storeInst :: Machine -> ExceptT Errors UnionM Machine
storeInst m = do
  PCValue x lx <- peekPC 0 m
...
add1Inst m = do
  ...
  p <- popN 2 m
  mrgReturn $ next $ push (MPCValue $ PCValue (x + y) $ con False) p

store1abInst :: Machine -> ExceptT Errors UnionM Machine
store1abInst m = do
  PCValue x _ <- peekPC 0 m
  y <- peekPC 1 m
  p <- popN 2 m
...
store1bInst :: Machine -> ExceptT Errors UnionM Machine
store1bInst m = do
  PCValue x lx <- peekPC 0 m
  PCValue y ly <- peekPC 1 m
  p <- popN 2 m
...
jump1abInst :: Machine -> ExceptT Errors UnionM Machine
jump1abInst m = do
  PCValue x _ <- peekPC 0 m
  p <- pop m
  mrgReturn $ goto (PCValue x $ con False) p

callInst :: PCValue -> PCValue -> Machine -> ExceptT Errors UnionM Machine
