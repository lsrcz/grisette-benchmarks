...
takeOnlyEnough :: (Mergeable a) => [a] -> SymInteger -> CBMCExceptT Errors UnionM [a]
takeOnlyEnough l i =
  mrgIf
    (i ==~ 0)
    (return [])
...
dropOnlyEnough :: (Mergeable a) => [a] -> SymInteger -> CBMCExceptT Errors UnionM [a]
dropOnlyEnough l i =
  mrgIf
    (i ==~ 0)
    (return l)
...
replace :: (Mergeable a) => [a] -> SymInteger -> a -> CBMCExceptT Errors UnionM [a]
...
insert :: (Mergeable a) => [a] -> SymInteger -> a -> CBMCExceptT Errors UnionM [a]
insert l i v =
  mrgIf
    (i ==~ 0)
    (return $! v : l)
