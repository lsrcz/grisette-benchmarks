...
tyassert :: SymBool -> CBMCExceptT BonsaiError UnionM ()
...
tyMatch ::
  ...
  LetPolyTree ->
  CBMCExceptT BonsaiError UnionM t
tyMatch = bonsaiMatchCustomError BonsaiTypeError

typeCompatible :: LetPolyTree -> LetPolyTree -> CBMCExceptT BonsaiError UnionM ()
...
isValidName :: BonsaiError -> SymWordN LetPolyWidth -> CBMCExceptT BonsaiError UnionM ()
isValidName err sym =
  symFailIfNot err $
    foldl
      ( \acc v ->
...
derefTy :: LetPolyTree -> CBMCExceptT BonsaiError UnionM (UnionM LetPolyTree)
derefTy = tyMatch [letPolyLiteral "ref" *= placeHolder ==> return]

typer' :: LetPolyTree -> Env LetPolyWidth LetPolyTree -> CBMCExceptT BonsaiError UnionM (UnionM LetPolyTree)
typer' tree env =
  tyMatch
...
typer :: LetPolyTree -> CBMCExceptT BonsaiError UnionM (UnionM LetPolyTree)
...
getRefEnv :: Integer -> RefEnv -> CBMCExceptT BonsaiError UnionM (UnionM LetPolyValue)
...
type EvalType =
  ...
  RefEnv ->
  LetPolyTree ->
  CBMCExceptT BonsaiError UnionM (UnionM LetPolyValue, RefEnv)
...
eval :: LetPolyTree -> CBMCExceptT BonsaiError UnionM (UnionM LetPolyValue)
eval tree = do
...
execLetPoly :: LetPolyTree -> CBMCExceptT BonsaiError UnionM (UnionM LetPolyValue)
execLetPoly tree = do
