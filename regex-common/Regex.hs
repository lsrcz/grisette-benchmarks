...
data Patt
  ...
  deriving (ToSym ConcPatt, EvaluateSym, Mergeable) via (Default Patt)

-- Creating regex sketch using 'Fresh' monad. (The monad used by 'SymGen' class in the paper, or 'GenSym' class in the current Grisette version)
freshPrim :: Fresh (UnionM Patt)
freshPrim = chooseFresh [PrimPatt 'd', PrimPatt 'c', PrimPatt 'b', PrimPatt 'a', EmptyPatt]

binFreshPrim :: (UnionM Patt -> UnionM Patt -> Fresh (UnionM Patt)) -> Fresh (UnionM Patt)
binFreshPrim f = freshPrim >>= \p1 -> freshPrim >>= \p2 -> f p1 p2

seqOrAlt :: Fresh (UnionM Patt)
seqOrAlt = binFreshPrim (\l r -> chooseFresh [SeqPatt l r, AltPatt l r])

sketchGen :: Fresh Patt
sketchGen = do
  s1 <- seqOrAlt
  f1 <- freshPrim
...
toCoroU ::
  (RegexSynth tag, SimpleMergeable (PattCoroType tag)) =>
  proxy tag ->
  UnionM Patt ->
  PattCoroType tag
  ...
  let constraints = conformsFirst tag coro reg <$> strs
   ...
   in do
        ...
        case solveRes of
          ...
          Right mo -> return $ Just $ evaluateSymToCon mo patt
