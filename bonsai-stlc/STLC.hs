...
isAvailableNameNode node = foldl (\acc v -> node .== v .|| acc) (con False) availableNames
  ...
  bonsaiMatchCustomError
    ...
    [ stlcLiteral "one" ==> mrgReturn (mrgReturn intTy),
      ...
      ((stlcLiteral "lambda" *= (placeHolder *= placeHolder)) *= placeHolder)
        ==> ( \nm ty expr -> do
                n <- lift nm
                _ <- symAssertTransformableError BonsaiTypeError (isAvailableNameNode n)
                  ...
                  BonsaiNode _ body -> do
                    ...
                    case bres of
                      BonsaiNode funcArgTy funcResTy -> do
                        argTy <- typer' .# arg # env
                        _ <- symAssertTransformableError BonsaiTypeError (argTy .== funcArgTy)
                        mrgReturn funcResTy
...
applyBuiltin (STLCBuiltin v) arg =
  mrgIf
    ...
    ( mrgIf
        (Just v .== (con <$> terminalToBV stlcSyntax "tl"))
        ( do
            a <- lift arg
            case a of
              STLCList lv -> do
...
applyBuiltin (STLCPartiallyAppliedBuiltin v arg1) arg2 =
  mrgIf
    (Just v .== (con <$> terminalToBV stlcSyntax "+"))
    ( do
        a <- lift arg1
        b <- lift arg2
        case (a, b) of
    ...
    ( mrgIf
        (Just v .== (con <$> terminalToBV stlcSyntax "cons"))
        ( do
            a <- lift arg1
            b <- lift arg2
            case (a, b) of
    ...
    then throwError BonsaiRecursionError
      ...
      bonsaiMatchCustomError
        ...
        [ stlcLiteral "one" ==> mrgReturn (uSTLCInt 1),
          ...
          stlcLiteral "hd" ==> mrgReturn (uSTLCBuiltin (con (fromJust (terminalToBV stlcSyntax "hd")))),
          stlcLiteral "tl" ==> mrgReturn (uSTLCBuiltin (con (fromJust (terminalToBV stlcSyntax "tl")))),
          stlcLiteral "cons" ==> mrgReturn (uSTLCBuiltin (con (fromJust (terminalToBV stlcSyntax "cons")))),
          stlcLiteral "+" ==> mrgReturn (uSTLCBuiltin (con (fromJust (terminalToBV stlcSyntax "+")))),
          ((stlcLiteral "lambda" *= (placeHolder *= placeHolder)) *= placeHolder)
            ==> ( \nm _ expr -> do
                    l <- lift nm
                    symAssertTransformableError BonsaiExecError (isAvailableNameNode l)
...
execStlc tree = do
  symAssertTransformableError BonsaiTypeError (matchStlcSyntax tree "term")
