...
tyassert = symAssertTransformableError BonsaiTypeError
...
isValidName err sym =
  symAssertTransformableError err $
    foldl
      ( \acc v ->
          acc
            ||~ Just sym
...
data LetPolyValue
  ...
  deriving (SEq, EvaluateSym) via (Default LetPolyValue)

instance Mergeable LetPolyValue where
  rootStrategy =
    SortedStrategy
      ( \case
          LetPolyInt _ -> 0 :: Int
...
$(makeUnionWrapper "u" ''LetPolyValue)
...
instance SimpleMergeable RefEnv where
  mrgIte cond (RefEnv t) (RefEnv f) = RefEnv $ go t f
    where
      go [] [] = []
      go [] ((fi, fv) : fs) = (fi, mrgIf cond mrgNothing fv) : go [] fs
      go ((ti, tv) : ts) [] = (ti, mrgIf cond tv mrgNothing) : go ts []
      go tl@((ti, tv) : ts) fl@((fi, fv) : fs)
        | ti > fi = (ti, mrgIte cond tv mrgNothing) : go ts fl
        | ti == fi = (ti, mrgIte cond tv fv) : go ts fs
        | otherwise = (fi, mrgIte cond mrgNothing fv) : go tl fs
...
execLetPoly tree = do
  symAssertTransformableError BonsaiTypeError (matchLetPolySyntax tree "term")
